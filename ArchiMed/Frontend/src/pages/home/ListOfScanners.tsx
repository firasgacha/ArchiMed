...
export default function ListOfScanners() {
  ...
  const data2 = useMemo(() => ScannersListData, [ScannersListData]);
  ...
  const [scannerDescription, setscannerDescription] = useState(String);
  const [scannerDate, setscannerDate] = useState(String);
  ...
  const deleteScanner = async () => {
    selectedFlatRows.map(async (row) => {
      await axios.delete(`Scanner/${row.original.scannerId}`)
        .then((res) => {
          fetchData();
        }).catch((err) => {
          console.log(err);
  ...
  const editScanner = async () => {
    await axios.put(`Scanner/${scannerId}`,
      {
        "scannerId": scannerId,
        "description": scannerDescription,
        "created": scannerDate,
  ...
  const EditFunction = () => {
    if (selectedFlatRows.length === 1) {
      selectedFlatRows.map(async (row) => {
        setscannerId(row.original.scannerId);
        setscannerDescription(row.original.description);
        setscannerDate(row.original.created);
              ...
              <div className="bg-white rounded-md shadow fixed overflow-y-auto sm:h-auto w-10/12 md:w-8/12 lg:w-1/2 2xl:w-2/5">
                ...
                <div className="px-4 md:px-10 pt-6 md:pt-12 md:pb-4 pb-7">
                  <form>
                    ...
                    <div className="flex items-center space-x-9 mt-8">
                      <textarea placeholder="Scanner description" defaultValue={scannerDescription} onChange={(e) => setscannerDescription(e.target.value)} className="text-center bg-transparent border border-gray-300  pl-3 py-3 shadow-sm rounded text-sm focus:outline-none focus:border-indigo-700 resize-none placeholder-gray-500 text-gray-500 " rows={5} cols={100} />
                    </div>
                    <div className="flex justify-center text-center items-center space-x-9 mt-3">
          ...
          <div className="container mx-auto bg-white rounded">
            <div className="flex justify-between border-b border-gray-300 py-5 bg-white">
              <div className="flex mx-auto xl:w-full xl:mx-0 items-center">
                ...
                <svg onClick={() => setshowAddScanners(!showAddScanners)} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                ...
                <svg onClick={() => deleteScanner()} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 ml-3">
          ...
          <div className="mx-auto container">
            <div className="flex items-center justify-center h-full w-full">
              <div className="bg-white rounded-md shadow fixed overflow-y-auto sm:h-auto w-auto md:w-auto lg:w-auto 2xl:w-auto">
                ...
                <div className="px-4 md:px-10 pt-6 md:pt-12 md:pb-4 pb-7">
                  <div className="flex">
                    <div className="max-w-[1000px] h-auto mt-10 mb-10 mr-10 ml-10">
